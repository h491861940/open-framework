...
import com.open.framework.workflow.model.debugger.BreakpointRepresentation;
import com.open.framework.workflow.model.debugger.DebuggerService;
import com.open.framework.workflow.model.editor.mapper.*;
 ...
 */
@Service
public class RuntimeFlowService {
    ...
    public JSONObject getDebuggerModel(String processInsId) {
        if (runtimeService.createProcessInstanceQuery().processInstanceId(processInsId).count() > 0 ) {
            return getInsModel(processInsId);
        }
        return getHisModel(processInsId);
    }

    public JSONObject getInsModel(String processInsId) {
        ...
        if (processInstance == null) {
            throw new RuntimeException("No process instance found with id " + processInsId);
        }
        ...
        if (pojoModel == null || pojoModel.getLocationMap().isEmpty()) {
            throw new RuntimeException("Process definition could not be found with id " + processInstance.getProcessDefinitionId());
        }

        // Fetch process-instance activities
        List<HistoricActivityInstance> activityInstances = historyService.createHistoricActivityInstanceQuery().processInstanceId(processInsId).list();
        ...
        List<Job> jobs = managementService.createJobQuery().processInstanceId(processInsId).list();
        if (CollectionUtils.isNotEmpty(jobs)) {
            List<Execution> executions = runtimeService.createExecutionQuery().processInstanceId(processInsId).list();
        ...
        ObjectNode displayNode = processProcessElements(pojoModel, completedElements, currentElements, breakpoints, processInsId);
        ...
        String jsonValue=displayNode.toString();
        JSONObject jsonObject=JsonUtil.parseObject(jsonValue);
        return jsonObject;
    }

    public JSONObject getDefModel(String processDefId) {

        BpmnModel pojoModel = repositoryService.getBpmnModel(processDefId);

        if (pojoModel == null || pojoModel.getLocationMap().isEmpty()) {
            throw new RuntimeException("Process definition could not be found with id " + processDefId);
        }
        ObjectNode displayNode=processProcessElements(pojoModel, null, null, Collections.EMPTY_LIST, "");
        String jsonValue=displayNode.toString();
        JSONObject jsonObject=JsonUtil.parseObject(jsonValue);
        return jsonObject;
    }

    public JSONObject getHisModel(String processInsId) {
        ...
        ObjectNode displayNode = processProcessElements(pojoModel, completedActivityInstances, currentActivityinstances, Collections.EMPTY_LIST, processInsId);
        String jsonValue=displayNode.toString();
        JSONObject jsonObject=JsonUtil.parseObject(jsonValue);
        return jsonObject;
    }
